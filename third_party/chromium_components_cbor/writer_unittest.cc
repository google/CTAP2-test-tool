...
TEST(CBORWriterTest, TestWriteUint) {
  ...
  static const UintTestCase kUintTestCases[] = {
      ...
      {10, std::string("\x0a")},
      {23, std::string("\x17")},
      {24, std::string("\x18\x18")},
      {25, std::string("\x18\x19")},
      {100, std::string("\x18\x64")},
      {1000, std::string("\x19\x03\xe8")},
      {1000000, std::string("\x1a\x00\x0f\x42\x40", 5)},
      {0xFFFFFFFF, std::string("\x1a\xff\xff\xff\xff")},
      {0x100000000, std::string("\x1b\x00\x00\x00\x01\x00\x00\x00\x00", 9)},
      {std::numeric_limits<int64_t>::max(),
       std::string("\x1b\x7f\xff\xff\xff\xff\xff\xff\xff")}};
...
TEST(CBORWriterTest, TestWriteNegativeInteger) {
  static const struct {
    ...
    const std::string cbor;
      ...
      {-23LL, std::string("\x36")},
      {-24LL, std::string("\x37")},
      {-25LL, std::string("\x38\x18")},
      {-100LL, std::string("\x38\x63")},
      {-1000LL, std::string("\x39\x03\xe7")},
      {-4294967296LL, std::string("\x3a\xff\xff\xff\xff")},
      {-4294967297LL, std::string("\x3b\x00\x00\x00\x01\x00\x00\x00\x00", 9)},
      {std::numeric_limits<int64_t>::min(),
       std::string("\x3b\x7f\xff\xff\xff\xff\xff\xff\xff")},
  };
...
TEST(CBORWriterTest, TestWriteBytes) {
  ...
  static const BytesTestCase kBytesTestCases[] = {
      {{}, std::string("\x40")},
      {{0x01, 0x02, 0x03, 0x04}, std::string("\x44\x01\x02\x03\x04")},
  };
...
TEST(CBORWriterTest, TestWriteString) {
  ...
  static const StringTestCase kStringTestCases[] = {
      {"", std::string("\x60")},
      {"a", std::string("\x61\x61")},
      {"IETF", std::string("\x64\x49\x45\x54\x46")},
      {"\"\\", std::string("\x62\x22\x5c")},
      {"\xc3\xbc", std::string("\x62\xc3\xbc")},
      {"\xe6\xb0\xb4", std::string("\x63\xe6\xb0\xb4")},
      {"\xf0\x90\x85\x91", std::string("\x64\xf0\x90\x85\x91")}};
...
TEST(CBORWriterTest, TestWriteArray) {
  ...
  EXPECT_THAT(cbor.value(),
              testing::ElementsAreArray(kArrayTestCaseCbor,
                                        ABSL_ARRAYSIZE(kArrayTestCaseCbor)));
}
...
TEST(CBORWriterTest, TestWriteMap) {
  ...
  EXPECT_THAT(cbor.value(),
              testing::ElementsAreArray(kMapTestCaseCbor,
                                        ABSL_ARRAYSIZE(kMapTestCaseCbor)));
}
...
TEST(CBORWriterTest, TestWriteMapWithArray) {
  ...
  EXPECT_THAT(cbor.value(),
              testing::ElementsAreArray(kMapArrayTestCaseCbor,
                                        ABSL_ARRAYSIZE(kMapArrayTestCaseCbor)));
}
...
TEST(CBORWriterTest, TestWriteNestedMap) {
  ...
  EXPECT_THAT(cbor.value(),
              testing::ElementsAreArray(kNestedMapTestCase,
                                        ABSL_ARRAYSIZE(kNestedMapTestCase)));
}
...
TEST(CBORWriterTest, TestSignedExchangeExample) {
  ...
  EXPECT_THAT(cbor.value(), testing::ElementsAreArray(
                                kSignedExchangeExample,
                                ABSL_ARRAYSIZE(kSignedExchangeExample)));
}
...
TEST(CBORWriterTest, TestWriteSimpleValue) {
  static const struct {
    ...
    const std::string cbor;
  } kSimpleTestCase[] = {{Value::SimpleValue::FALSE_VALUE, std::string("\xf4")},
                         {Value::SimpleValue::TRUE_VALUE, std::string("\xf5")},
                         {Value::SimpleValue::NULL_VALUE, std::string("\xf6")},
                         {Value::SimpleValue::UNDEFINED, std::string("\xf7")}};
