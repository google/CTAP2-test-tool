...
class CBOR_EXPORT Value {
 public:
  ...
  using MapValue = std::map<Value, Value, Less>;
  ...
  explicit Value(const BinaryValue& in_bytes);
  ...
  explicit Value(const std::string& in_string, Type type = Type::STRING);
  ...
  std::string GetBytestringAsString() const;
